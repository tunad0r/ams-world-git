  ...
  return (
    <div className="min-h-screen flex flex-col justify-between">
      <div className="max-w-[1400px] w-full mx-auto px-6 py-12 flex-grow space-y-12">
        ...
        {/* Barra de Pesquisa e Filtros (Removido o botão de Físicas) */}
      ...
      {/* MODAL DE DOWNLOAD COM BOTÕES CONDICIONAIS DE CARSET E TEMPLATE */}
      {selectedMod && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/95 backdrop-blur-md transition-opacity duration-300">
          <div className="bg-[#121318] border border-[#2d313d] w-full max-w-2xl rounded overflow-hidden shadow-2xl relative flex flex-col md:flex-row">
            ...
            <div className="p-8 md:w-1/2 flex flex-col justify-between space-y-6">
              <button 
                ...
                className="absolute top-4 right-4 text-gray-400 hover:text-white font-bold text-lg cursor-pointer animate-pulse-slow"
              >
                ✕
              </button>
              ...
              <div className="space-y-3 pt-4 border-t border-[#1c1e24]">
                ...
                {/* Botão de Download Principal do Mod */}
                <a 
                ...
                {/* BOTÕES OPCIONAIS: Carset e Templates (Só aparecem se houver dados cadastrados) */}
                {(selectedMod.carsetLink || selectedMod.templateLink) && (
                  <div className="flex flex-col sm:flex-row gap-3 pt-1">
                    {selectedMod.carsetLink && (
                      <a 
                        href={selectedMod.carsetLink} 
                        target="_blank" 
                        rel="noreferrer"
                        className="flex-1 py-2.5 bg-[#1a1b22] hover:bg-neutral-800 border border-[#2d313d] text-white font-black text-[9px] tracking-wider rounded text-center block uppercase transition-colors"
                      >
                        📦 {t.btnCarset}
                      </a>
                    )}
                    {selectedMod.templateLink && (
                      <a 
                        href={selectedMod.templateLink} 
                        target="_blank" 
                        rel="noreferrer"
                        className="flex-1 py-2.5 bg-[#1a1b22] hover:bg-neutral-800 border border-[#2d313d] text-white font-black text-[9px] tracking-wider rounded text-center block uppercase transition-colors"
                      >
